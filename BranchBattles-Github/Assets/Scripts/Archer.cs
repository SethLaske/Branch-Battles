...
//Unused class, currently soldier is versatile enough to handle all fighters
public class Archer : Soldier
{
    //private float FrontStoppingDistance = 1;
    //private float AdvanceIndicatorDistance = 1.3f;
    //private float RetreatIndicatorDistance = .85f;
    ...
    public override void Wait()
    {
        if (Target != null) //Transitions for when there is a target
        {
            ...
            {   //We want to reach the target and no one is infront of us
                ...
                Debug.Log("Need to advance forward my max target is: " + (General.RallyPoint + (AgroRange * Team)) + "And the enemy is at " + Target.transform.position.x);
                State = "Walk";
            }
            else
            {  //If we reach this point, then it has a target it cant reach. If the target is out of range, this covers it. If its falsely removing it, the Collision detector will add it back
    ...
    public override void Walk()
    {
        ...
        if (Target == null && General.RallyPoint * Team < (transform.position.x * Team))
        {   //There is no target and they only respect the rally point
            State = "Retreat";
        }
